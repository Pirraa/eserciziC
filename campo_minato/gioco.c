//struttura gioco: stato iniziale-> mossa1 -> stato2 -> mossa2 -> stato3
//la -> è una transizione da uno stato all'altro, quindi una funzione che applica delle modifiche
...
int sconfitta(Stato *ps)
{
    ...
    for(i=0; i<NRIGHE;i++)
        for(j=0; j<NCOLONNE; j++)
            if(ps->campo_minato[i][j].mina_presente && !ps->campo_minato[i][j].coperta)
            return 1;
    return 0;
}
    ...
    for(i=0; i<NRIGHE;i++)
        for(j=0; j<NCOLONNE; j++)
        {
            ...
            /*if(rnd_dbl(0.0,1.0)<=PROBMINA){
                ...
                //c[i][j]='*';
                (*ps).campo_minato[i][j].mina_presente=1;
            }else{
                //avviene con probabilità 1-probmina=0.8
                //c[i][j]=' ';
                (*ps).campo_minato[i][j].mina_presente=0;
            }*/
  ...
  for (riga = i - 1; riga <= i + 1; riga++)
    for (colonna = j - 1; colonna <= j + 1; colonna++)
      if (riga >= 0 && riga < NRIGHE && colonna >= 0 && colonna < NCOLONNE &&
          c[riga][colonna].mina_presente && c[riga][colonna].mina_presente)
        nmine++;
  return nmine;
}
...
void leggiMossa(Mossa *pm)
{
    ...
    do{
        printf("Mossa?\n");
        scanf("%s",stringa);
    }while(strlen(stringa)!=2 || stringa[0]<'a' || stringa[0]>'a'+NRIGHE-1 || stringa[1] < '1' || stringa[1]>'1'+NCOLONNE-1);
    pm->riga=stringa[0]-'a';
    pm->colonna = stringa[1]-'1';
}

void presentaStato(Stato *ps)
{
    int i,j;
    for(i=0; i<NRIGHE;i++)
    {
